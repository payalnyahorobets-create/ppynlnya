...
const sanitizeNomenclature = (items) =>
  (items || [])
...
const sanitizeSalesMonths = (months) => {
  const cleaned = {};
  Object.entries(months || {}).forEach(([month, rows]) => {
    cleaned[month] = Array.isArray(rows)
      ? rows.filter((row) => row && row.sku).map((row) => ({
...
const restoreState = () => {
  ...
  state.nomenclature = sanitizeNomenclature(payload.nomenclature);
  ...
  state.salesMonths = sanitizeSalesMonths(payload.salesMonths);
...
const mergeNomenclature = (items, scope) => {
  const bySku = new Map(sanitizeNomenclature(state.nomenclature).map((item) => [item.sku, item]));
  items.forEach((item) => {
...
const refreshAll = () => {
  state.nomenclature = sanitizeNomenclature(state.nomenclature);
  state.salesMonths = sanitizeSalesMonths(state.salesMonths);
